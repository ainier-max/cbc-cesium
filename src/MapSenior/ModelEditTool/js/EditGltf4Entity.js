...
class EditGltf4Entity {
   ...
   * @param {*} gltf
   */
  constructor(viewer, gltf, d, r, myOption) {
  ...
  initParam() {
    this.removeAllTools();
    let gltf = this._gltf;
    const length = this.myOption.length;
          ...
          $this._handler.setInputAction(function (movement) {
            ...
            switch ($this._currentPick._name) {
              case "model_edit_xArrow":
                _tx = (endDegree.lng - downDegree.lng) / 2;
                break;
              case "model_edit_yArrow":
                _ty = (endDegree.lat - downDegree.lat) / 2;
                break;
              case "model_edit_zArrow":
                _tz = (-$this._dStep / 4) * _yPix;
                break;
  ...
  updateModel(params, _tx, _ty, _tz, _rx, _ry, _rz) {
    ...
    if (this._coordCircle) {
      ...
      // console.log("this._currentPick._name", this._currentPick._name);
      if (this._currentPick._name.indexOf("Arrow") >= 0) {
        this.myOption.callback(this._gltf, "Arrow");
      } else {
        this.myOption.callback(this._gltf, "Circle");
      }

      //console.log("this._gltf", this._gltf);
